                    ...
                    <div class="2xl:col-span-5 xl:col-span-4 lg:col-span-4 md:col-span-4 sm:col-span-1">
                        <div class="flex items-center space-x-5">
                            <img src="{{ asset('storage/' . auth()->user()->profile_image) }}"
                                id="profile_image" alt="profile_image" class="h-24 w-24 rounded-full border bg-white" />
                            <div class="input-group">
                                <label for="profile_image" class="input-label">Profile</label>
                                <button type="button" onclick="document.getElementById('profile_image-input').click()" class="btn-primary-sm w-fit flex items-center space-x-1.5">
                                    <span>Select Image</span>
                                    <i data-feather="upload"></i>
                                </button>
                                <input hidden type="file" accept="image/jpeg, image/jpg, image/png" class="input-box-sm"
                                    name="profile_image" id="profile_image-input" onchange="handleProfilePreview(event)">
                                @error('profile_image')
    ...
    <script>
        ...
        const handleProfilePreview = (event) => {
            ...
            if (event.target.files.length == 0) {
                ...
                document.getElementById('profile_image').src = URL.createObjectURL(event.target.files[0])
            }
        }
